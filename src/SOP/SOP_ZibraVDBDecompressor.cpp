...
namespace Zibra::ZibraVDBDecompressor
{
    ...
    OP_ERROR SOP_ZibraVDBDecompressor::cookMySop(OP_Context& context)
    {
        ...
        const exint frameIndex = (seqInfo.sequenceFlags & CE::Decompression::SEQUENCE_FLAG_FILE_PER_FRAME)
            ? m_DecompressorManager.GetFrameRange().start
            : evalInt(FRAME_PARAM_NAME, 0, context.getTime());
        ...
        res = m_DecompressorManager.DecompressFrame(frameMemory, frameProxy, &vdbGrids);
        if (ZIB_FAILED(res))
        {
