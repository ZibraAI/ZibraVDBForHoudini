...
namespace Zibra::InspectVolume
{
    ...
    ROP_RENDER_CODE ROP_InspectVolume::renderFrame(const fpreal time, UT_Interrupt* boss)
    {
        ...
        addMessage(ROP_MESSAGE, frameMessage.c_str());
        ...
        GA_FOR_ALL_PRIMITIVES(gdp, prim)
        {
            if (prim->getTypeId() == GEO_PRIMVDB)
            {
...

                {
                    ...
                    addMessage(ROP_MESSAGE, message.c_str());
                }

                {
                    UT_VDBType vdbType = vdbPrim->getStorageType();
                    std::string message = "Type: "s + UTvdbType(vdbType);
                    addMessage(ROP_MESSAGE, message.c_str());
                }
                ...
                if (!bbox.empty())
                {
                    ...
                    {
                        addMessage(ROP_MESSAGE, "Can't sample this grid type");
                    }
                }
            }
        }
        ...
        addMessage(ROP_MESSAGE, vdbCountMessage.c_str());
